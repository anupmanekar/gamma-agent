...
class VanillaNodeAdapter:
    ...
    def check_for_browser_actions(self, state: VanillaGraphState) -> Command[Literal["extract_browser_actions", "human_get_instructions"]]:
        ...
        if response.content.strip().lower() == "false":
            return Command(goto="human_get_instructions")
        else:
    ...
    def human_check_for_additional_instructions(self, state: VanillaGraphState) -> Command[Literal["human_get_instructions", "human_review_browser_actions"]]:

        human_response = interrupt(
            {
                "question": "Do you have any additional instructions?",
                "options": ["yes", "no"],
            })
